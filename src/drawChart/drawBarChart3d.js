    ...
    canvas.addEventListener('mousemove', (event) => {
        ...
        values.forEach((row, rowIndex) => {
            ...
            activeValues.forEach((value, activeIndex) => {
                ...
                // Check for side wall
                for (let n = depth; n > 0; n--) {
                    if (mouseX >= barX + n && mouseX <= barX + barThickness + n && mouseY >= barY - n && mouseY <= barY + barHeight - n) {
                        tooltip.style.left = `${event.clientX + 10}px`;
                        tooltip.style.top = `${event.clientY + 10}px`;
                        tooltip.style.display = 'block';
                        tooltip.innerHTML = `${activeColumns[activeIndex].label}:  ${value}`;
                ...
                // Check for top wall
                if (mouseX >= barX && mouseX <= barX + barThickness && mouseY >= barY - depth && mouseY <= barY) {
                    tooltip.style.left = `${event.clientX + 10}px`;
                    tooltip.style.top = `${event.clientY + 10}px`;
                    tooltip.style.display = 'block';
                    tooltip.innerHTML = `${activeColumns[activeIndex].label}:  ${value}`;
